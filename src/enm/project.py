...
from .toolchain import Compiler, detect_compilers, resolve_compiler, resolve_toolchain_compiler
...
def _write_cmake_initial_cache(
    root: Path, manifest: dict[str, Any], compiler: Compiler | None = None
) -> Path:
    ...
    if compiler is not None:
        ...
        lines.append(f'set(CMAKE_CXX_COMPILER "{escaped}" CACHE FILEPATH "Generated by ENM" FORCE)\n')
...
def _resolve_project_sdk(root: Path, store: StateStore, force: bool = False) -> InstalledSdk:
    """Resolve the SDK for a project, with optional fallback to the active SDK."""
    manifest = load_manifest(root)
    version = manifest.get("eui", {}).get("version")
    if not version:
        raise ReleaseError(f"{MANIFEST} does not pin an EUI-NEO release")
    ...
    try:
        ...
        if not force:
            raise ReleaseError(
                f"{exc}. Run 'enm configure --force' (or 'enm build --force') to use the active SDK instead."
            ) from exc
        try:
            sdk = store.active()
        except ReleaseError as active_exc:
            raise ReleaseError(
                f"{exc}; no active SDK is available to use with --force"
            ) from active_exc
        print(f"warning: project SDK {version} is not installed; using active SDK {sdk.version}")

    if not supports_external_apps(sdk):
        raise ReleaseError(
            f"EUI-NEO SDK {sdk.version} exports neither eui_neo_configure_app() nor eui::neo; "
            "install and select a compatible release SDK"
        )
    return sdk
...
def configure(root: Path, store: StateStore, extra: list[str] | None = None, force: bool = False) -> int:
    manifest = load_manifest(root)
    sdk = _resolve_project_sdk(root, store, force=force)

    # Validate EUI-NEO SDK version matches the project pin.
    pinned_version = manifest.get("eui", {}).get("version")
    if pinned_version and sdk.version != pinned_version and not force:
        raise ReleaseError(
            f"active EUI-NEO SDK is {sdk.version} but project requires {pinned_version}. "
            "Run 'enm configure --force' to ignore the SDK mismatch."
        )

    # Resolve toolchain constraint and inject compiler into the initial cache.
    compiler = resolve_toolchain_compiler(manifest)
    toolchain = manifest.get("toolchain") or {}
    if toolchain.get("compiler") and compiler is None and not force:
        raise ReleaseError(
    ...
    initial_cache = _write_cmake_initial_cache(root, manifest, compiler)
    command = [
        "cmake",
        "-C",
        str(initial_cache),
    ...
    use_msvc_generator = is_windows and (compiler is None or compiler.family == "msvc")
    if use_msvc_generator:
        ...
        raise ReleaseError(
            "non-MSVC toolchain selected on Windows requires Ninja. "
            "Install Ninja or switch the project toolchain to msvc."
        )
    return _run([*command, *(extra or [])], root, sdk, extra_environment)


def build(root: Path, store: StateStore, extra: list[str] | None = None, force: bool = False) -> int:
    manifest = load_manifest(root)
    sdk = _resolve_project_sdk(root, store, force=force)
    pinned_version = manifest.get("eui", {}).get("version")
    if pinned_version and sdk.version != pinned_version and not force:
        raise ReleaseError(
            f"active EUI-NEO SDK is {sdk.version} but project requires {pinned_version}. "
            "Run 'enm build --force' to ignore the SDK mismatch."
        )
    return _run(
...
def test(root: Path, store: StateStore, extra: list[str] | None = None, force: bool = False) -> int:
    manifest = load_manifest(root)
    sdk = _resolve_project_sdk(root, store, force=force)
    pinned_version = manifest.get("eui", {}).get("version")
    if pinned_version and sdk.version != pinned_version and not force:
        raise ReleaseError(
            f"active EUI-NEO SDK is {sdk.version} but project requires {pinned_version}. "
            "Run 'enm test --force' to ignore the SDK mismatch."
        )
    build_dir = manifest.get("build_dir", "build/default")
    result = _run(
        [
